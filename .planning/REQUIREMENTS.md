...
- [ ] **PROF-01**: User can select a distinct Fallout 4 CBBE profile that does not reuse Skyrim CBBE or Skyrim UUNP defaults, multipliers, or inverted-slider behavior.
...
- [ ] **PROF-04**: User can generate templates, morphs, and BoS JSON with profile-specific behavior covered by tests for each bundled profile.
...
- [ ] **DIAG-01**: User can run a read-only project validation report that identifies profile, preset, target, NPC assignment, reference, and export-readiness issues.
- [ ] **DIAG-02**: User can inspect profile diagnostics showing slider coverage, unknown sliders, injected defaults, multipliers, inversions, and likely profile mismatch indicators.
- [ ] **DIAG-03**: User can preview NPC import results, including parsed rows, invalid lines, duplicates, charset fallback, and assignment effects before committing import changes.
