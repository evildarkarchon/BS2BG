...
v1.0 advances BS2BG from the completed M0-M7 port through profile correctness, workflow hardening, diagnostics, custom profile extensibility, automation, portable bundles, release trust, and audit gap closures for headless custom-profile generation and saved strategy replay.

## Milestone Summary

**Key Decisions:**

- Preserve Java output semantics and `.jbs2bg` compatibility as the trust contract.
- Keep unresolved-profile fallback neutral and non-blocking while deferring mismatch heuristics.
- Use request-scoped custom profile catalogs for GUI, CLI, and bundle outputs.
- Replay saved assignment strategies in BodyGen/all automation outputs while preserving BoS-only and no-strategy behavior.

**Issues Resolved:**

- Closed all 25 v1 requirements and all milestone audit gaps.
- Added distinct bundled Fallout 4 CBBE profile semantics and release-facing calibration context.
- Added workflow persistence, NPC filtering/scopes, diagnostics, custom profile management, CLI generation, portable bundles, release trust packaging, and deterministic strategy replay.

**Issues Deferred:**

- Phase 1 visual fallback-panel UAT scenario acknowledged as waived at close.
- Phase 2 real Avalonia filter/scope, large-dataset responsiveness, and packaged restart persistence UAT scenarios acknowledged as waived at close.
- Advanced modding and ecosystem integrations remain v2 candidates unless explicitly pulled forward.

**Technical Debt Incurred:**

- Fallout 4 profile calibration remains seed-quality until validated against community data.
- Cross-platform release package confidence remains future work beyond Windows-first packaging.

---

# Archived Roadmap: BS2BG Future Work
