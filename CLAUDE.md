...
- **`src/BS2BG.Core/`** — `netstandard2.1`, `LangVersion 13`. Pure domain + I/O, no Avalonia or platform deps. Subfolders:
  ...
  - `Export/` — `OutputArtifactPlanner` (authoritative paths/exact bytes) and `OutputArtifactCommitter` (atomic group commits).
...
- `src/BS2BG.Core/Export/OutputArtifactPlanner.cs` — authoritative output paths and bytes; BodyGen INI uses `\r\n`, while BoS JSON uses `\n` only with **no trailing newline**.
