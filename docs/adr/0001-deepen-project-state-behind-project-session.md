---
status: accepted; the "preserving Java 8" clause is superseded by ADR-0003
date: 2026-08-26
amended: 2026-08-28
---
...
> The "preserving Java 8" clause below is superseded by [ADR-0003](0003-java-25-baseline-for-the-packaged-application.md):
> the application baseline is stable, pinned Java 25 LTS with JavaFX 25 and no preview or incubator use. Every other
> decision here — the `ProjectSession` interface, immutable snapshots, `.jbs2bg` semantic compatibility, and the
> rejected options — remains accepted. [ADR-0002](0002-project-aggregate-internal-seam.md) builds on this decision by
> placing the immutable `Project` aggregate behind the same seam; it does not replace it.
