...
public sealed class WindowFileDialogService : IFileDialogService
{
    ...
    {
        if (owner?.StorageProvider.CanOpen != true) return null;

        var files = await owner.StorageProvider.OpenFilePickerAsync(new FilePickerOpenOptions
        {
            Title = "Open jBS2BG File", AllowMultiple = false, FileTypeFilter = new[] { CreateProjectFileType() }
        });
    ...
    public Task<string?> PickBodyGenExportFolderAsync(CancellationToken cancellationToken) =>
        PickFolderAsync("Export Templates and Morphs INI", cancellationToken);

    public Task<string?> PickBosJsonExportFolderAsync(CancellationToken cancellationToken) =>
        PickFolderAsync("Export BoS JSON files", cancellationToken);
