...
<#
.SYNOPSIS
    Builds, verifies, and smoke-tests the self-contained Windows x64 Workbench image (issues #98-#101 and gates).
...
.DESCRIPTION
    ...
    packaging checkpoint:
      ...
      6. Extracts that archive to a clean temporary location and runs tools/java25/smoke-app-image.ps1. The
         ...
         reduced motion, accessibility semantics, feedback, dialogs, New, centralized Open admission/progress/
         cancellation, recovered Save, Save As, malformed/failed-operation preservation, linked retry,
         active-job shutdown/resume, and bounded exit are exercised through Windows UI Automation.
